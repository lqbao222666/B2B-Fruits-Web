...
const submitReport = async () => {
  ...
  try {
    ...
    notify.success('Đã gửi báo cáo sự cố thành công! Admin sẽ xem xét và liên hệ lại.')
    emit('success')
    closeModal()
  } catch (error) {
...
<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
    <div class="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-fade-in-up">
      <!-- Header -->
      <div class="bg-[#FFF8E1] px-6 py-4 flex items-center justify-between border-b border-[#FFE082]">
        <h3 class="text-[#FF8F00] font-black text-lg flex items-center gap-2">
          <span class="material-symbols-outlined">report_problem</span>
          Báo cáo sự cố đơn hàng
        </h3>
        <button @click="closeModal" class="text-[#FF8F00] hover:bg-[#FFE082] rounded-full p-1 transition-colors">
      ...
      <div class="p-6 space-y-5">
        <div>
          <label class="block text-sm font-bold text-slate-700 mb-1.5">Chi tiết sự cố <span class="text-red-500">*</span></label>
          <textarea 
            v-model="reason" 
            rows="4" 
        ...
        <div>
          <label class="block text-sm font-bold text-slate-700 mb-1.5">Hình ảnh/Video minh chứng</label>
          <div class="grid grid-cols-4 gap-3">
            ...
            <label v-if="previewUrls.length < 4" class="aspect-square rounded-xl border-2 border-dashed border-slate-300 hover:border-[#FF8F00] hover:bg-[#FF8F00]/5 cursor-pointer flex flex-col items-center justify-center text-slate-400 hover:text-[#FF8F00] transition-colors">
              <span class="material-symbols-outlined mb-1">add_photo_alternate</span>
              <span class="text-[10px] font-bold">Thêm ảnh</span>
              <input type="file" multiple accept="image/*,video/*" class="hidden" @change="handleFileChange" />
            </label>
          </div>
          <p class="text-[11px] text-slate-500 mt-2">Đính kèm ảnh vườn bị hư hại hoặc lý do khác (Tối đa 4 file).</p>
        </div>
      </div>
