...
<template>
  <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full">
    <div>
      <select v-model="selectedProvinceCode" class="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl focus:outline-none focus:border-[#2E7D32] focus:ring-1 focus:ring-[#2E7D32] transition-colors appearance-none text-slate-700">
        <option value="" disabled>Tỉnh/Thành phố</option>
        <option v-for="p in provinces" :key="p.code" :value="p.code">{{ p.name }}</option>
      </select>
    </div>
    <div>
      <select v-model="selectedDistrictCode" :disabled="!selectedProvinceCode" class="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl focus:outline-none focus:border-[#2E7D32] focus:ring-1 focus:ring-[#2E7D32] transition-colors disabled:opacity-50 disabled:bg-slate-100 appearance-none text-slate-700">
        <option value="" disabled>Quận/Huyện</option>
        <option v-for="d in districts" :key="d.code" :value="d.code">{{ d.name }}</option>
      </select>
    </div>
    <div>
      <select v-model="selectedWardCode" :disabled="!selectedDistrictCode" class="w-full bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl focus:outline-none focus:border-[#2E7D32] focus:ring-1 focus:ring-[#2E7D32] transition-colors disabled:opacity-50 disabled:bg-slate-100 appearance-none text-slate-700">
