        ...
        <div ref="messagesContainer" class="flex-1 p-4 overflow-y-auto space-y-4">
          ...
          >
            ...
            >
              ...
              <div v-if="message.detected_category || message.detected_province || message.detected_standard || message.detected_price_range || message.detected_rating" class="mt-3 flex flex-wrap gap-2">
                <router-link
                  :to="`/products?${[
                    message.detected_category ? 'category=' + message.detected_category.id : '',
                    message.detected_province ? 'province=' + message.detected_province : '',
                    message.detected_standard ? 'standard=' + message.detected_standard : '',
                    message.detected_price_range ? 'price=' + message.detected_price_range : '',
                    message.detected_rating ? 'rating=' + message.detected_rating : ''
                  ].filter(Boolean).join('&')}`"
                ...
                >
                  ...
                  Xem {{ message.detected_category ? message.detected_category.name : 'sản phẩm' }} {{ message.detected_province ? 'ở ' + message.detected_province : '' }}
                </router-link>
              </div>

              <!-- Gợi ý sản phẩm/nhu cầu -->
              <div v-if="message.suggestions && message.suggestions.length > 0" class="mt-3 space-y-2">
                <div v-for="item in message.suggestions" :key="item.id" class="bg-white p-2.5 rounded border border-slate-200 shadow-sm text-xs text-slate-700">
                  <p class="font-semibold text-slate-900 mb-1 leading-tight">{{ item.ten }}</p>
                  <div class="space-y-0.5">
                    <p><span class="font-medium text-slate-500">Giá:</span> <span class="text-[#658a22] font-semibold">{{ item.gia }}</span></p>
                    <p><span class="font-medium text-slate-500">Khu vực:</span> {{ item.dia_chi }}</p>
                  </div>
                  <p class="mt-1.5 pt-1.5 border-t border-slate-100"><span class="font-medium text-slate-500">Liên hệ:</span> {{ item.lien_he }}</p>
                </div>
              </div>
            </div>
          </div>
