...
const setRating = (type: 'tong' | 'chatLuong' | 'dungHen' | 'thaiDo', val: number) => {
...
const submitRating = async () => {
  ...
  try {
    ...
    notify.error(error?.response?.data?.message || 'Không thể gửi đánh giá. Vui lòng thử lại.')
  } finally {
...
<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
    <div class="bg-white rounded-3xl w-full max-w-xl overflow-hidden shadow-2xl animate-fade-in-up">
      <!-- Header -->
      <div class="bg-emerald-50 px-6 py-4 flex items-center justify-between border-b border-emerald-100">
        ...
        <button @click="closeModal" class="text-slate-400 hover:text-slate-600 rounded-full p-1 transition-colors">
      ...
      <div class="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
        <!-- Thông tin sản phẩm -->
        <div v-if="order" class="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100">
          <span class="material-symbols-outlined text-emerald-600 bg-white p-2.5 rounded-xl border border-slate-200">agriculture</span>
          <div>
            <div class="font-bold text-slate-800 text-sm">
              {{ order.baiDang?.tieu_de || order.baiDang?.ten_nong_san || 'Đơn hàng nông sản' }}
            </div>
            <div class="text-xs text-slate-500">Mã đơn: #{{ order.ma_don_hang || order.donhang_id }}</div>
          </div>
        </div>

        <!-- Đánh giá Tổng thể -->
        <div class="text-center py-2 bg-amber-50/50 rounded-2xl border border-amber-100/80">
          <div class="text-sm font-bold text-slate-700 mb-2">Đánh giá chung sản phẩm</div>
          ...
          <div class="text-xs font-bold text-amber-600 mt-1">
            {{ diemTong === 5 ? 'Rất tuyệt vời (5/5)' : diemTong === 4 ? 'Hài lòng (4/5)' : diemTong === 3 ? 'Bình thường (3/5)' : diemTong === 2 ? 'Không hài lòng (2/5)' : 'Rất kém (1/5)' }}
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div class="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
            <div class="text-xs font-bold text-slate-600 mb-1">Chất lượng trái</div>
            <div class="flex justify-center gap-0.5">
              <button 
                ...
                class="text-lg"
                :class="s <= diemChatLuong ? 'text-amber-400' : 'text-slate-200'"
              >
                ★
              </button>
            </div>
          ...
          <!-- Đúng hẹn -->
          <div class="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
            <div class="text-xs font-bold text-slate-600 mb-1">Giao đúng hẹn</div>
            <div class="flex justify-center gap-0.5">
              <button 
                v-for="s in 5" 
                :key="s" 
          ...
          <!-- Thái độ -->
          <div class="bg-slate-50 p-3 rounded-xl border border-slate-100 text-center">
            <div class="text-xs font-bold text-slate-600 mb-1">Thái độ phục vụ</div>
            <div class="flex justify-center gap-0.5">
              <button 
                v-for="s in 5" 
                :key="s" 
        ...
        <!-- Upload ảnh đính kèm -->
        <div>
          <label class="block text-sm font-bold text-slate-700 mb-1.5">Hình ảnh thực tế (không bắt buộc)</label>
          <div class="grid grid-cols-4 gap-3">
            ...
            <label v-if="previewUrls.length < 4" class="aspect-square rounded-xl border-2 border-dashed border-slate-300 hover:border-emerald-500 hover:bg-emerald-50 cursor-pointer flex flex-col items-center justify-center text-slate-400 hover:text-emerald-600 transition-colors">
          ...
          <p class="text-[11px] text-slate-400 mt-2">Đánh giá kèm ảnh thực tế giúp các Doanh nghiệp khác và Nông dân tin tưởng hơn.</p>
        </div>
      </div>
