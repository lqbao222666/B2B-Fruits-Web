...
const initMap = (initLat?: number | null, initLng?: number | null) => {
  ...
  map = L.map("edit-post-map").setView(
  ...
  try {
    ...
    if (post.phanLoais && post.phanLoais.length > 0) {
      phanLoais.value = post.phanLoais.map((pl: any) => ({
        ...
        original_so_luong_con_lai: Number(
          pl.so_luong_con_lai ?? pl.so_luong_co,
        ),
      }));
    } else {
      phanLoais.value = [
        {
          ...
          so_luong_con_lai: Number(
            post.so_luong_con_lai || post.so_luong_co || 0,
          ),
    ...
    existingImages.value = Array.isArray(rawImgs)
      ? JSON.parse(JSON.stringify(rawImgs))
      : [];
...
const handleFileChange = (e: Event) => {
  ...
  if (target.files) {
    Array.from(target.files).forEach((file) => {
      const isFirst =
        existingImages.value.length === 0 && selectedImages.value.length === 0;
  ...
  if (type === "existing") {
    if (typeof existingImages.value[index] === "object") {
      ...
      existingImages.value[index] = {
        url: existingImages.value[index],
        is_main: true,
      };
    }
  } else {
...
const getFullUrl = (path: any) => {
  if (!path) return "";
  let imgPath =
    typeof path === "object" && (path.url || path.image_url)
      ? path.url || path.image_url
      : path;
  if (typeof imgPath !== "string") return "";
  return imgPath.startsWith("http")
    ? imgPath
    : `${import.meta.env.VITE_API_URL || "http://localhost:3000"}${imgPath.startsWith("/") ? "" : "/"}${imgPath}`;
};
</script>
...
<template>
  <div class="edit-post-root bg-[#f8fafc] py-12 px-4 min-h-screen font-sans">
    ...
    <div
      v-if="fetching"
      class="max-w-4xl mx-auto bg-white rounded-3xl p-12 shadow-sm border border-slate-100 flex flex-col items-center justify-center"
    >
      <div
        class="animate-spin rounded-full h-12 w-12 border-b-2 border-[#2E7D32] mb-4"
      ></div>
    ...
    >
      ...
      >
        <div
          class="w-14 h-14 bg-[#E3F2FD] rounded-2xl flex items-center justify-center shadow-inner"
        >
          <span class="text-3xl material-symbols-outlined text-[#1565C0]"
            >edit_square</span
          >
        </div>
        <div>
          ...
          <p class="text-slate-500 text-sm mt-1 font-medium">
            Chỉnh sửa thông tin, số lượng, giá và vị trí của lô hàng.
          </p>
        </div>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-8 relative z-10">
        ...
        <section>
          <div class="flex items-center justify-between mb-4">
            ...
            <button
              type="button"
              @click="addPhanLoai"
              class="flex items-center gap-1 text-sm font-bold text-[#2E7D32] hover:bg-[#e8f5e9] px-3 py-1.5 rounded-lg transition-colors"
            >
          ...
          <div class="space-y-4">
            <div
              ...
              :key="index"
              class="bg-slate-50/50 p-5 rounded-2xl border border-slate-100 relative group"
            >
              <button
                v-if="phanLoais.length > 1"
                type="button"
                @click="removePhanLoai(index)"
                class="absolute -top-3 -right-3 w-8 h-8 bg-red-100 text-red-600 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500 hover:text-white shadow-sm"
              >
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div class="md:col-span-1">
                  ...
                  <input
                    ...
                    type="text"
                    required
                    placeholder="VD: Loại 1, Trái lớn..."
          ...
          >
            ...
            <div v-if="tieuChuansList.length > 0">
              <label class="block text-sm font-bold text-slate-700 mb-2"
                >Tiêu chuẩn nông sản đạt được</label
              >
            ...
            <div class="mt-4 border-t border-slate-200 pt-4">
              ...
              >
                <span class="material-symbols-outlined text-sm"
                  >location_on</span
                >
                Tọa độ đã chọn: {{ form.latitude.toFixed(6) }},
            ...
            <div v-if="existingImages.length > 0">
              ...
              <div class="flex flex-wrap gap-3">
                <div
                  ...
                  :key="idx"
                  class="relative text-xs font-bold bg-white px-3 py-2 rounded-xl border border-slate-200 flex flex-col items-center gap-1 group/img"
                  :class="
                    img.is_main ? 'ring-2 ring-[#2E7D32] bg-[#c8e6c9]' : ''
                  "
                >
                  <!-- Badge ảnh chính -->
                  <span
                    v-if="img.is_main"
                    class="absolute -top-2 -right-2 bg-yellow-400 text-yellow-900 text-[10px] px-2 py-0.5 rounded-full shadow-sm flex items-center gap-1 z-10"
                  >
                  ...
                  <div
                    class="w-24 h-24 rounded-lg overflow-hidden relative group"
                  >
                    <img
                      :src="getFullUrl(img)"
                  ...
                  <div class="flex gap-1 mt-1">
                    <button
                      v-if="!img.is_main"
                      @click.prevent="setMainImage('existing', idx)"
                    ...
                    <button
                      @click.prevent="removeExistingImage(idx)"
                  ...
                  >
                    ...
                    >
                      ...
                      <div class="flex gap-1 mt-1">
                        <button
                          v-if="!imgObj.isMain"
                          @click.prevent="setMainImage('new', index)"
                          class="px-2 py-1 bg-white text-[#2E7D32] rounded border border-[#2E7D32]/20 hover:bg-[#2E7D32] hover:text-white transition-colors"
                          title="Đặt làm ảnh chính"
                        >
                          Chọn chính
        ...
        >
          ...
          <button
            type="submit"
            :disabled="loading"
            class="w-full sm:w-auto px-10 py-3.5 bg-[#2E7D32] hover:bg-[#1B5E20] text-white font-bold rounded-xl transition-all shadow-md active:scale-95 flex items-center justify-center gap-2"
          >
            <span
              v-if="loading"
