...
const fetchSavedLocations = async () => {
  ...
  try {
    ...
    if (savedLocations.value.length > 0) {
      ...
      moveToWarehouse(
        Number(savedLocations.value[0].latitude),
        Number(savedLocations.value[0].longitude),
      );
    }
  } catch (err) {
    console.error(err);
...
const updateCalculations = () => {
  ...
  if (
    ...
    distanceKm.value = haversineDistance(
      farmerLat,
      farmerLng,
      warehouseLocation.value.lat,
      warehouseLocation.value.lng,
    );
...
const updatePhanLoaiQty = (phanloai_id: number, val: number) => {
  const pl = post.value?.phanLoais?.find(
    (p: any) => p.phanloai_id === phanloai_id,
  );
  ...
  if (nextVal > maxStock) {
    notify.info(
      `Số lượng tối đa còn lại của ${pl?.ten_phan_loai} là ${maxStock}`,
    );
    return;
  }
...
const moveToWarehouse = (lat: number, lng: number) => {
  ...
  if (map) {
    ...
    warehouseMarker = L.marker([lat, lng])
      .addTo(map)
      .bindPopup("Kho hàng nhận của bạn")
      .openPopup();

    const farmerLat = post.value?.latitude
      ? Number(post.value.latitude)
      : 10.762622;
    const farmerLng = post.value?.longitude
      ? Number(post.value.longitude)
      : 106.660172;
    ...
    routeLine = L.polyline(
      ...
      { color: "#2E7D32", weight: 4, dashArray: "8, 8" },
    ).addTo(map);
    map.fitBounds(
      [
        [farmerLat, farmerLng],
...
const initMap = () => {
  ...
  L.marker([farmerLat, farmerLng])
    .addTo(map)
    .bindPopup("Vị trí lô hàng (Nông dân)")
    .openPopup();
...
const handleThuongLuong = () => {
  ...
  const selectedItems =
    post.value?.phanLoais?.filter(
      (pl: any) => Number(phanLoaiQuantities.value[pl.phanloai_id]) > 0,
    ) || [];
  ...
  if (!warehouseLocation.value) {
    notify.error(
      "Vui lòng chọn vị trí kho nhận hàng trên bản đồ hoặc tìm kiếm địa chỉ!",
    );
    return;
  }
...
<template>
  <main class="mx-auto max-w-[1200px] w-full px-4 py-8 font-sans relative">
    ...
    >
      <span
        class="material-symbols-outlined text-5xl text-[#2E7D32] animate-bounce mb-4"
        >agriculture</span
      >
      <p class="font-bold uppercase tracking-widest text-sm text-[#2E7D32]">
        Đang tải thông tin...
      </p>
    </div>
    ...
    <div v-else>
      ...
      <div
        v-if="post.trang_thai === 'da_xoa'"
        class="mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg flex items-start gap-3"
      >
        <span class="material-symbols-outlined text-red-500 mt-0.5"
          >warning</span
        >
        <div>
          <h3 class="font-bold text-red-800">
            Sản phẩm này đã ngừng kinh doanh/bị xóa bởi người bán
          </h3>
          <p class="text-sm text-red-700 mt-1">
            Bạn chỉ có thể xem lại thông tin sản phẩm để hoàn tất phần giao dịch
            còn lại của đơn hàng hiện tại.
          </p>
        </div>
      </div>
      ...
      >
        <RouterLink
          to="/"
          class="hover:text-[#2E7D32] transition-colors flex items-center gap-1"
        >
      ...
      <!-- Main Layout -->
      <div
        class="grid grid-cols-1 lg:grid-cols-12 gap-10 bg-white p-6 md:p-10 rounded-[2.5rem] shadow-sm border border-slate-200"
      >
        <!-- Cột trái: Hình ảnh -->
        <div class="lg:col-span-5 space-y-4">
          <!-- Ảnh chính -->
          <div
            class="aspect-square w-full bg-[#E8F5E9]/50 rounded-3xl overflow-hidden flex items-center justify-center border-2 border-[#2E7D32]/10 relative group cursor-zoom-in"
          ...
          >
            ...
            >
              <span
                class="material-symbols-outlined opacity-0 group-hover:opacity-100 text-white drop-shadow-md text-4xl transition-opacity"
                >zoom_in</span
              >
            </div>
          </div>
          ...
          >
            <div
              ...
              class="w-20 h-20 flex-shrink-0 rounded-2xl overflow-hidden cursor-pointer border-2 hover:border-[#2E7D32] transition-colors relative"
              :class="idx === 0 ? 'border-[#2E7D32]' : 'border-transparent'"
            >
              <img :src="getImageUrl(img)" class="w-full h-full object-cover" />
              <span
                v-if="img.is_main"
                class="absolute bottom-1 left-1 bg-[#2E7D32] text-white text-[8px] px-1 rounded uppercase font-bold"
                >Chính</span
              >
            </div>
          </div>
          ...
          <div v-if="post.latitude" class="pt-4 space-y-3">
            <div class="flex items-center justify-between">
              ...
              >
                <span class="material-symbols-outlined text-[#2E7D32] text-sm"
                  >map</span
                >
            ...
            <div class="flex gap-2">
              <input
                ...
                type="text"
                placeholder="Nhập tên kho / địa chỉ (VD: Kho Cần Thơ)..."
                class="w-full text-xs p-2.5 rounded-xl border border-slate-200 outline-none focus:border-[#2E7D32]"
              />
              <button
                @click="handleSearchWarehouse"
                :disabled="isSearching"
                class="bg-[#2E7D32] hover:bg-[#1B5E20] text-white px-3 rounded-xl flex items-center justify-center transition-colors"
              >
                <span
                  v-if="isSearching"
            ...
            <div
              id="view-map"
              class="w-full h-[220px] bg-slate-100 rounded-2xl border-2 border-slate-200 overflow-hidden relative z-0"
            ></div>
            <p class="text-[11px] text-slate-400 italic text-center">
              * Click lên bản đồ để chọn vị trí kho nhận hàng của bạn
            </p>
          </div>
        </div>
        ...
        <div class="lg:col-span-7 flex flex-col gap-6">
          <div>
            <div
              class="flex items-center gap-2 mb-2 text-[#2E7D32] font-bold text-sm"
            >
              <span class="material-symbols-outlined text-[18px]"
                >verified</span
              >
              {{ post.ten_nong_san }} &bull; Cập nhật:
              {{ formatDate(post.updated_at) }}
            </div>
            <h1
              class="text-3xl md:text-4xl font-black text-slate-900 leading-tight mb-4"
            >
              {{ post.tieu_de }}
            </h1>
            <div class="flex flex-wrap items-center gap-3">
              <div
                class="flex items-center gap-2 bg-slate-50 border border-slate-100 px-4 py-2 rounded-xl text-slate-600 font-medium inline-flex"
              >
          ...
          <div
            v-if="post.phanLoais && post.phanLoais.length > 0"
            class="space-y-3 mt-2"
          >
            <div class="flex justify-between items-center">
              <h3 class="text-sm font-bold text-slate-800">
                Các Phân Loại Sản Phẩm Trong Lô Này
              </h3>
              <span class="text-xs text-[#2E7D32] font-bold"
                >* Nhập số lượng cho nhiều loại cùng lúc</span
              >
            </div>

            <div class="space-y-3">
              <div
                v-for="pl in post.phanLoais"
                :key="pl.phanloai_id"
                ...
                :class="
                  Number(phanLoaiQuantities[pl.phanloai_id]) > 0
                    ? 'border-[#2E7D32] bg-green-50/30 shadow-sm'
              ...
              >
                <div class="space-y-1">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="font-black text-[#d00000] text-base"
                      >{{ formatPrice(pl.gia)
                  ...
                  <p class="text-xs text-slate-500">
                    Còn lại trong kho:
                    <strong>{{ pl.so_luong_con_lai }}</strong>
                    {{ post.don_vi_tinh }}
                  </p>
                </div>
                ...
                <div class="flex items-center gap-2 self-end sm:self-center">
                  <button
                    @click="updatePhanLoaiQty(pl.phanloai_id, -1)"
                    :disabled="
                      !phanLoaiQuantities[pl.phanloai_id] ||
                      Number(phanLoaiQuantities[pl.phanloai_id]) <= 0
                    "
                    class="w-8 h-8 rounded-full bg-slate-200 hover:bg-slate-300 flex items-center justify-center font-black disabled:opacity-30 transition-colors"
                  >
                    -
                  </button>
                  <input
                  ...
                  <button
                    @click="updatePhanLoaiQty(pl.phanloai_id, 1)"
                    :disabled="
                      Number(phanLoaiQuantities[pl.phanloai_id]) >=
                      pl.so_luong_con_lai
                    "
                    class="w-8 h-8 rounded-full bg-slate-200 hover:bg-slate-300 flex items-center justify-center font-black disabled:opacity-30 transition-colors"
                  >
          ...
          <div
            class="bg-[#e8f5e9]/70 border-2 border-[#2E7D32]/20 p-5 rounded-2xl space-y-3"
          >
            <h3
              class="text-xs font-black text-[#2E7D32] uppercase tracking-wider flex items-center gap-1.5"
            >
            ...
            <div class="space-y-2 text-sm">
              <div class="flex justify-between items-center text-slate-600">
                <span
                  >Tổng khối lượng chọn ({{ totalQuantitySelected }}
                  {{ post.don_vi_tinh }}):</span
                >
              ...
              <div class="flex justify-between items-center text-slate-600">
                <span class="flex items-center gap-1">
                  Khoảng cách kho:
                  <span v-if="distanceKm > 0" class="font-bold text-[#2E7D32]"
                    >({{ distanceKm.toFixed(1) }} km)</span
                  >
                  <span v-else class="text-xs text-slate-400 font-normal"
                    >(Chưa chọn vị trí kho)</span
                  >
                </span>
                <span class="font-bold text-slate-800">{{
                  distanceKm > 0 ? formatPrice(shippingFee) : "0 ₫"
                }}</span>
              </div>
              <div
                class="pt-2 border-t border-[#2E7D32]/20 flex justify-between items-center text-base"
              >
                <span class="font-black text-slate-900"
                  >TỔNG TIỀN THANH TOÁN (ĐÃ CỘNG VC):</span
                >
          ...
          <div class="bg-slate-50 p-6 rounded-2xl border border-slate-100 mt-2">
            <h3
              class="text-sm font-black text-slate-800 uppercase tracking-wide mb-3"
            >
              Mô tả chi tiết
            </h3>
            <p
              class="text-slate-600 text-sm leading-relaxed whitespace-pre-wrap"
            >
              {{ post.mo_ta || "Chưa có mô tả cho sản phẩm này." }}
            </p>
          </div>

          <!-- ĐÁNH GIÁ & BÌNH LUẬN TỪ NGƯỜI MUA TRƯỚC (LƯU TRỮ KHI TÁI SỬ DỤNG BÀI ĐĂNG) -->
          <div
            class="bg-white p-6 rounded-2xl border border-slate-200 mt-4 shadow-sm"
          >
            <div
              class="flex flex-wrap items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-100"
            >
              <div>
                <h3
                  class="text-base font-black text-slate-800 uppercase tracking-wide flex items-center gap-2"
                >
                ...
                <p class="text-xs text-slate-500 mt-1">
                  Lưu trữ toàn bộ đánh giá uy tín của các giao dịch hoàn thành
                  trước đó đối với bài đăng này
                </p>
              </div>

              <!-- Điểm trung bình bài đăng -->
              <div
                class="flex items-center gap-3 bg-amber-50 px-4 py-2 rounded-2xl border border-amber-200"
              >
                <div class="text-2xl font-black text-amber-600">
                  {{ Number(post.diem_trung_binh || 0).toFixed(1) }}
                </div>
                <div>
                  <div class="flex text-amber-400 text-sm">
                    <span v-for="s in 5" :key="s">{{
                      s <= Math.round(Number(post.diem_trung_binh || 0))
                        ? "★"
                        : "☆"
                    }}</span>
                  </div>
                  <div class="text-[11px] font-bold text-slate-500">
                    {{ danhGiaList.length }} lượt đánh giá
                  </div>
                </div>
              </div>
            </div>
            ...
            <!-- Danh sách rỗng -->
            <div
              v-else-if="danhGiaList.length === 0"
              class="py-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200"
            >
              <span
              ...
              <p class="text-xs text-slate-400 mt-1">
                Các doanh nghiệp hoàn thành giao dịch đầu tiên sẽ để lại bình
                luận ở đây.
              </p>
            </div>
            ...
            <div v-else class="space-y-6">
              <div
                v-for="review in danhGiaList"
                :key="review.danhgia_id"
                class="bg-slate-50 p-5 rounded-2xl border border-slate-100 space-y-3"
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div
                      class="w-9 h-9 rounded-full bg-emerald-600 text-white font-bold text-sm flex items-center justify-center shadow-sm uppercase"
                    >
                  ...
                  <!-- Số sao -->
                  <div
                    class="flex items-center gap-1 bg-white px-3 py-1 rounded-xl border border-slate-200 text-amber-500 font-bold text-xs"
                  >
                ...
                <!-- Tiêu chí con -->
                <div
                  class="flex flex-wrap gap-3 text-[11px] text-slate-600 bg-white/60 p-2 rounded-xl border border-slate-100"
                >
                  <span v-if="review.diem_chat_luong"
                    >Chất lượng:
                    <strong class="text-amber-600"
                      >{{ review.diem_chat_luong }}★</strong
                    ></span
                  >
                  <span v-if="review.diem_dung_hen"
                    >Đúng hẹn:
                    <strong class="text-amber-600"
                      >{{ review.diem_dung_hen }}★</strong
                    ></span
                  >
                  <span v-if="review.diem_thai_do"
                    >Thái độ:
                    <strong class="text-amber-600"
                      >{{ review.diem_thai_do }}★</strong
                    ></span
                  >
                </div>
                ...
                <div
                  ...
                  class="flex gap-2 pt-1 overflow-x-auto"
                >
                  <img
                ...
                <div
                  v-if="review.tra_loi"
                  class="mt-3 bg-emerald-50/80 p-3.5 rounded-xl border border-emerald-100 text-xs text-emerald-900 space-y-1"
                >
                  <div
                    class="font-bold flex items-center gap-1.5 text-emerald-800"
                  >
                ...
                <div
                  v-else-if="
                    user &&
                    (user.id === post.nguoi_dang_id ||
                      user.user_id === post.nguoi_dang_id)
                  "
          ...
          <!-- Hành động -->
          <div
            class="mt-auto pt-6 flex flex-col sm:flex-row gap-4 border-t border-slate-100"
          >
            <button
              @click="contactSeller"
              class="w-full sm:w-1/2 flex items-center justify-center gap-2 bg-white hover:bg-slate-50 text-slate-700 border-2 border-slate-200 px-6 py-4 rounded-2xl font-bold transition-all shadow-sm active:scale-95"
            >
