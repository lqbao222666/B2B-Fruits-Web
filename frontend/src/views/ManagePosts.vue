...
const getStatusBadge = (status: string) => {
  switch (status) {
    ...
    default:
      return { text: status, class: "bg-slate-100 text-slate-800" };
  }
};
...
const filteredPosts = computed(() => {
  ...
  return posts.value.filter((p) => {
    if (
      typeof filterCategoryId.value === "string" &&
      filterCategoryId.value.startsWith("cl_")
    ) {
...
const getImageUrl = (images: any) => {
  ...
  if (typeof firstImage === "string") {
    return firstImage.startsWith("http")
      ? firstImage
      : `${import.meta.env.VITE_API_URL || "http://localhost:3000"}${firstImage.startsWith("/") ? "" : "/"}${firstImage}`;
  }
  ...
  <div class="manage-posts-root bg-[#f8fafc] py-10 px-4 min-h-screen font-sans">
    <div class="max-w-6xl mx-auto">
      ...
      >
        <div>
          <h1
            class="text-3xl font-black text-slate-800 uppercase tracking-tight"
          >
            Quản lý <span class="text-[#2E7D32]">Bài đăng</span>
          </h1>
          <p class="text-slate-500 font-medium text-sm mt-1">
            Quản lý lô hàng nông sản bạn đang rao bán
          </p>
        </div>
        <div class="flex items-center gap-3">
          <select
            v-model="filterCategoryId"
            class="px-4 py-2 border border-slate-200 rounded-xl bg-white text-sm font-medium text-slate-700 outline-none focus:border-[#2E7D32] transition-colors cursor-pointer"
          >
            <option value="">Tất cả danh mục</option>
            <template v-for="cl in chungLoais" :key="'cl_' + cl.chungloai_id">
              <option
                :value="'cl_' + cl.chungloai_id"
          ...
          <RouterLink
            to="/create-post"
            class="bg-[#2E7D32] hover:bg-[#1B5E20] text-white px-6 py-3 rounded-xl font-bold transition-all shadow-md active:scale-95 flex items-center gap-2"
          >
      ...
      <div v-if="loading" class="space-y-4">
        <div
          v-for="i in 3"
          :key="i"
          class="animate-pulse bg-white p-6 rounded-2xl border border-slate-100 flex gap-6"
        >
      ...
      >
        <div
          class="w-24 h-24 bg-[#e8f5e9] text-[#2E7D32] rounded-full flex items-center justify-center mx-auto mb-4"
        >
        ...
        <p class="text-slate-500 mb-6">
          Bạn chưa tạo bài đăng bán nông sản nào trên hệ thống.
        </p>
        <button
          @click="router.push('/create-post')"
          class="px-8 py-3 bg-[#2E7D32] hover:bg-[#1B5E20] text-white font-bold rounded-xl transition-all shadow-sm"
        >
          Tạo Bài Đăng Ngay
        </button>
      </div>
      ...
      <div v-else class="space-y-4">
        <div
          ...
          :key="post.baidang_id"
          class="bg-white rounded-2xl p-5 md:p-6 border border-slate-100 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row gap-6 relative overflow-hidden group"
        >
          <!-- Image -->
          <div
            class="w-full md:w-32 h-40 md:h-32 rounded-xl overflow-hidden flex-shrink-0 bg-slate-100 relative"
          >
            <img
              :src="getImageUrl(post.images)"
              :alt="post.ten_nong_san"
              class="w-full h-full object-cover"
            />
            <div
              class="absolute inset-0 border border-black/5 rounded-xl"
            ></div>
          </div>
          ...
          <div class="flex-1 flex flex-col justify-between">
            <div>
              <div class="flex items-start justify-between gap-4">
                <h3
                  class="text-lg font-bold text-slate-800 line-clamp-2 leading-tight"
                >
                  {{ post.tieu_de }}
                </h3>
                <span
                  class="px-3 py-1 text-xs font-bold rounded-lg border whitespace-nowrap"
                  :class="getStatusBadge(post.trang_thai).class"
                >
                  {{ getStatusBadge(post.trang_thai).text }}
                </span>
              </div>
              <p class="text-sm font-semibold text-[#2E7D32] mt-1">
                {{ post.ten_nong_san }} &bull;
                {{ post.danhMuc?.ten_danh_muc || "Danh mục trống" }}
              </p>

              <div
                class="grid grid-cols-2 md:flex md:flex-wrap gap-x-6 gap-y-2 mt-4 text-sm text-slate-600"
              >
                <div class="flex items-center gap-1.5">
                  <span
                    ...
                    >payments</span
                  >
                  <span class="font-bold text-[#d00000]"
            ...
            <!-- Actions -->
            <div
              class="flex items-center gap-3 mt-5 pt-5 border-t border-slate-50 justify-end"
            >
              <button
                @click="toggleVisibility(post)"
                class="px-4 py-2 text-sm font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors flex items-center gap-1.5"
              >
                <span class="material-symbols-outlined text-[16px]">{{
                  post.trang_thai === "an" ? "visibility" : "visibility_off"
                }}</span>
                {{ post.trang_thai === "an" ? "Hiện bài" : "Ẩn bài" }}
              </button>

              <button
                @click="router.push(`/edit-post/${post.baidang_id}`)"
                class="px-4 py-2 text-sm font-bold text-[#1565C0] bg-[#E3F2FD] hover:bg-[#BBDEFB] rounded-lg transition-colors flex items-center gap-1.5"
              >
              ...
              <button
                @click="stopProvidingPost(post.baidang_id)"
                class="px-4 py-2 text-sm font-bold text-[#E65100] bg-[#FFF3E0] hover:bg-[#FFE0B2] rounded-lg transition-colors flex items-center gap-1.5"
              >
                <span class="material-symbols-outlined text-[16px]"
                  >stop_circle</span
                >
              ...
              <button
                @click="deletePost(post.baidang_id)"
                class="px-4 py-2 text-sm font-bold text-[#C62828] bg-[#FFEBEE] hover:bg-[#FFCDD2] rounded-lg transition-colors flex items-center gap-1.5"
              >
                <span class="material-symbols-outlined text-[16px]"
                  >delete</span
                >
