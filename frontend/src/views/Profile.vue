  ...
  try {
    ...
    if (accountRes) {
      ...
      if (accountRes.ngay_sinh) {
        profile.dob = new Date(accountRes.ngay_sinh)
          .toISOString()
          .split("T")[0];
      }
      ...
      const isNongDan =
        ...
        roleStr === "nong dan";
      const isDoanhNghiep =
        roleStr === "doanh_nghiep" ||
        roleStr === "doanh nghiệp" ||
        roleStr === "doanh nghiep";
      ...
      if (isNongDan) {
        try {
          ...
          if (ndRes.data) {
            ...
            profile.thong_tin_xuat_hoa_don = nd.thong_tin_xuat_hoa_don
              ? nd.thong_tin_xuat_hoa_don.noi_dung
              : "";
            ...
            // Map common fields
            if (!profile.fullName || profile.fullName === "Người dùng mới")
              profile.fullName = nd.ho_ten || profile.fullName;
            if (!profile.phone)
              profile.phone = nd.so_dien_thoai || profile.phone;
            if (!profile.address) {
              const fullAddress = [
                nd.dia_chi_cu_the,
                nd.huyen_xa,
          ...
          console.error("Error fetching nong_dan data:", e);
        }
        ...
        try {
          ...
          if (dnRes.data) {
            ...
            // Map common fields
            if (!profile.fullName || profile.fullName === "Người dùng mới")
              profile.fullName = dn.nguoi_dai_dien || profile.fullName;
            if (!profile.phone)
              profile.phone = dn.so_dien_thoai || profile.phone;
            if (!profile.address) {
              const fullAddress = [dn.dia_chi, dn.tinh_thanh]
                .filter(Boolean)
                .join(", ");
              profile.address = fullAddress || profile.address;
            }
          ...
          console.error("Error fetching doanh_nghiep data:", e);
        }
      }
    }
  } catch (error: any) {
...
const selectDefaultAvatar = async (avatarUrl: string) => {
  ...
  try {
    const responseAccount = await Account.updateAvatar(
      accountInfo.id,
      avatarUrl,
    );
  ...
  try {
    ...
    if (isNongDan) {
      await api.patch(`/nong-dan/${profile.accountId}`, {
        ...
        huyen_xa:
          [profile.quan_huyen, profile.phuong_xa].filter(Boolean).join(", ") ||
          "",
        dia_chi_cu_the: profile.dia_chi_cu_the || "",
        thong_tin_xuat_hoa_don: profile.thong_tin_xuat_hoa_don
          ? { noi_dung: profile.thong_tin_xuat_hoa_don }
          : undefined,
        giay_phep_urls: profile.giay_phep_urls || [],
        so_cmnd_cccd: profile.so_cmnd_cccd || "",
        dien_tich_ha: profile.dien_tich_ha
          ? Number(profile.dien_tich_ha)
          : undefined,
      ...
      await api.patch(`/doanh-nghiep/${profile.accountId}`, {
        ...
        tinh_thanh: profile.tinh_thanh || profile.address || "Chưa cập nhật",
        dia_chi:
          [profile.dia_chi_cu_the, profile.phuong_xa, profile.quan_huyen]
            .filter(Boolean)
            .join(", ") || "",
            ...
            <div class="flex flex-col items-center text-center">
              <div class="relative inline-block mb-5">
                <div
                  ...
                  :class="{
                    'cursor-pointer hover:border-[#658a22]/30': isEditing,
                  }"
                ...
                >
                  ...
                  <div
                    v-else
                    class="w-full h-full bg-[#eef4e6] flex items-center justify-center text-[#658a22] text-3xl font-bold uppercase"
                  >
                    {{
                      accountInfo.email
                        ? accountInfo.email.substring(0, 2)
                        : "TP"
                    }}
                  </div>
              ...
              >
                ...
                >
                  <button
                    ...
                    class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 bg-slate-50 hover:bg-slate-100 rounded-full p-1.5 transition-colors"
                  >
                  ...
                  <p class="text-sm text-slate-500 mb-6">
                    Bạn có thể chọn một ảnh có sẵn hoặc tải lên từ thiết bị của
                    mình.
                  </p>
          ...
          >
            ...
            <div class="p-8 space-y-6">
              ...
              <template
                v-if="
                  accountInfo.role &&
                  (accountInfo.role.toLowerCase() === 'nong_dan' ||
                    accountInfo.role.toLowerCase() === 'nông dân' ||
                    accountInfo.role.toLowerCase() === 'nong dan')
                "
              >
                <div class="mt-8 border-t border-slate-100 pt-6">
                  <h4 class="font-bold text-md text-slate-800 mb-4">
                    Thông tin Kinh Doanh (Dành cho Nông Dân/Người bán)
                  </h4>
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1.5">
                      <label
                    ...
                    <div class="space-y-1.5 md:col-span-2">
                      <label
                        class="block text-xs font-bold text-slate-600 uppercase tracking-wider ml-1"
                        >Khu vực (Tỉnh/Quận/Xã)</label
                      >
                      <div
                        :class="{
                          'opacity-50 pointer-events-none': !isEditing,
                        }"
                      >
                        <LocationSelector
              ...
              <template
                v-if="
                  accountInfo.role &&
                  (accountInfo.role.toLowerCase() === 'doanh_nghiep' ||
                    accountInfo.role.toLowerCase() === 'doanh nghiệp' ||
                    accountInfo.role.toLowerCase() === 'doanh nghiep')
                "
              >
                <div class="mt-8 border-t border-slate-100 pt-6">
                  <h4 class="font-bold text-md text-slate-800 mb-4">
                    Thông tin Doanh Nghiệp (Dành cho Người mua)
                  </h4>
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1.5">
                      <label
