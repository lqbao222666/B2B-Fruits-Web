...
<template>
  <div class="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-white via-white to-[#eef4e6] relative overflow-hidden font-inter">

    <div class="absolute inset-0 nature-pattern opacity-30 pointer-events-none"></div>
    <div class="absolute -top-24 -left-24 w-96 h-96 bg-[#658a22]/10 rounded-full blur-3xl"></div>
    <div class="absolute -bottom-24 -right-24 w-96 h-96 bg-[#658a22]/5 rounded-full blur-3xl"></div>

    <div class="w-full max-w-[480px] bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-slate-100 z-10 p-8">

      <button @click="goBack" class="flex items-center gap-1 text-slate-400 hover:text-[#658a22] transition-colors mb-6 text-sm font-semibold w-fit">
        <span class="material-symbols-outlined text-[18px]">arrow_back</span>
        Quay lại hồ sơ
      </button>

      <div class="text-center mb-8">
        <div class="size-16 bg-[#eef4e6] rounded-2xl flex items-center justify-center mx-auto mb-4 text-[#658a22]">
          <span class="material-symbols-outlined text-3xl">lock_reset</span>
        </div>
        <h1 class="text-2xl font-extrabold tracking-tight text-slate-900 mb-2">Đổi Mật Khẩu</h1>
        <p class="text-slate-500 text-sm">Tạo mật khẩu mới mạnh mẽ để bảo vệ tài khoản của bạn</p>
      </div>

      <form class="space-y-5" @submit.prevent="handleSubmit">

        <div>
          <label class="block text-sm font-bold text-slate-700 mb-1.5">Mật khẩu hiện tại</label>
          <div class="relative flex items-center">
            ...
            >
              <span class="material-symbols-outlined text-[20px]">{{ showOldPassword ? 'visibility_off' : 'visibility' }}</span>
            </button>
          </div>
        </div>
        ...
        <div>
          <label class="block text-sm font-bold text-slate-700 mb-1.5">Xác nhận mật khẩu mới</label>
          <div class="relative flex items-center">
            <input
