...
const handleLogin = async () => {
  ...
  try {
    ...
    if (res?.token) {
      ...
      notify.success(
        `Chào mừng trở lại, ${res.user?.full_name || res.user?.email}!`,
      );
...
const goStep2 = () => {
  ...
  if (!regForm.so_dien_thoai.trim()) {
    notify.error("Vui lòng nhập số điện thoại");
    return;
  }
  if (!regForm.ma_so_thue.trim()) {
    notify.error("Vui lòng nhập Mã số thuế");
    return;
  }
  if (!regForm.ten_co_so_kd.trim()) {
    notify.error("Vui lòng nhập Tên theo giấy phép kinh doanh");
    return;
  }
  if (!regForm.doi_tuong_dang_ky.trim()) {
    notify.error("Vui lòng chọn Đối tượng đăng ký");
    return;
  }
  if (!regForm.tinh_thanh.trim()) {
    notify.error("Vui lòng nhập tỉnh thành");
    return;
  }
  if (regForm.password.length < 8) {
    notify.error("Mật khẩu tối thiểu 8 ký tự");
    return;
  }
...
const handleRegister = async () => {
  ...
  try {
    await auth.RegisterNongDan({
      ...
      huyen_xa: [regForm.quan_huyen, regForm.phuong_xa]
        .filter(Boolean)
        .join(", "),
      dia_chi_cu_the: regForm.dia_chi_cu_the,
      giay_phep_urls: regForm.giay_phep_urls,
      thong_tin_xuat_hoa_don: regForm.thong_tin_xuat_hoa_don
        ? { noi_dung: regForm.thong_tin_xuat_hoa_don }
        : null,
    });
    notify.success("Đăng ký thành công! Tài khoản đã được tự động phê duyệt (Mức 2).");
    setTimeout(() => {
      mode.value = "login";
      step.value = 1;
    }, 1200);
...
<template>
  <div class="nd-root">
    <!-- ═══════════════ LEFT PANEL ═══════════════ -->
    <div
      class="nd-panel cursor-pointer hover:opacity-95 transition-opacity"
      @click="router.push('/auth/doanh-nghiep')"
    >
      <!-- Decorative blobs -->
      ...
      <RouterLink to="/" class="nd-logo">
        ...
        >
          ...
          />
          <path
            d="M24 14C24 14 30 20 30 26C30 29.314 27.314 32 24 32C20.686 32 18 29.314 18 26C18 20 24 14 24 14Z"
            fill="rgba(0,100,60,0.4)"
          />
        </svg>
      ...
      <div class="nd-image-slot">
        ...
        <p class="nd-placeholder-hint">
          Thêm ảnh minh họa<br />nông dân tại đây
        </p>
      </div>
      ...
      <div class="nd-tagline">
        ...
        <p>
          Nền tảng B2B đưa sản phẩm của bạn<br />đến hàng trăm doanh nghiệp thu
          mua
        </p>
      </div>
      ...
      <div class="nd-form-inner">
        ...
        <template v-if="mode === 'login' && !isForgot">
          <div class="nd-form-header">
            <div class="nd-role-badge nd-role-farmer">
              🌾 Nông Dân / Người Bán
            </div>
            <h1 class="nd-form-title">Đăng nhập</h1>
            <p class="nd-form-sub">
              Tiếp tục quản lý sản phẩm và đơn hàng của bạn
            </p>
          </div>
          ...
          <form @submit.prevent="handleLogin" class="nd-form">
            <div class="nd-field">
              ...
              <div class="nd-input-wrap">
                <span class="nd-input-icon"
                  ><span class="material-symbols-outlined text-current"
                    >mail</span
                  ></span
                >
                <input
            ...
            <div class="nd-field">
              <div class="nd-label-row">
                ...
                <RouterLink to="/forgot-password" class="nd-forgot"
          ...
          <p class="nd-switch-text">
            Chưa có tài khoản?
            <a href="#" @click.prevent="switchToRegister" class="nd-link"
              >Đăng ký ngay</a
            >
          </p>
        </template>
        ...
        <template v-if="mode === 'login' && isForgot">
          ...
          <p class="nd-switch-text">
            <a href="#" @click.prevent="isForgot = false" class="nd-link"
          ...
          <form @submit.prevent="goStep2" class="nd-form">
            <div class="nd-field">
              <label class="nd-label"
                >Họ và tên <span class="nd-required">*</span></label
              >
              <div class="nd-input-wrap">
                <span class="nd-input-icon"
                  ><span class="material-symbols-outlined text-current"
            ...
            <div class="nd-field">
              <label class="nd-label"
                >Email <span class="nd-required">*</span></label
              >
              <div class="nd-input-wrap">
                <span class="nd-input-icon"
                  ><span class="material-symbols-outlined text-current"
            ...
            <div class="nd-field">
              <label class="nd-label"
                >Số điện thoại <span class="nd-required">*</span></label
              >
              <div class="nd-input-wrap">
                <span class="nd-input-icon"
                  ><span class="material-symbols-outlined text-current"
            ...
            <div class="nd-field">
              <label class="nd-label"
                >Khu vực (Tỉnh/Quận/Xã)
                <span class="nd-required">*</span></label
              >
              <LocationSelector
            ...
            <div class="nd-field">
              ...
              <div class="nd-input-wrap">
                ...
                >
                <input
                  v-model="regForm.ten_co_so_kd"
                  type="text"
                  required
                  placeholder="Nhập tên theo GPKD"
                  class="nd-input"
                />
              </div>
            </div>
            ...
            <div class="nd-field">
              <label class="nd-label"
                >Đối tượng đăng ký <span class="nd-required">*</span></label
              >
              <div class="nd-input-wrap">
                <select
                  v-model="regForm.doi_tuong_dang_ky"
                  class="nd-input"
                  required
            ...
            <div class="nd-field">
              ...
              <div class="nd-input-wrap">
                ...
                >
                <input
                  v-model="regForm.dia_chi_cu_the"
                  type="text"
            ...
            <div class="nd-field">
              <label class="nd-label"
                >Giấy phép kinh doanh (Tối đa 3 file)</label
              >
              <div
                class="nd-input-wrap"
                style="
              ...
              >
                <input
                  ...
                  @change="uploadGiayPhep"
                />
                <div
                  v-if="regForm.giay_phep_urls.length > 0"
                  style="margin-top: 10px"
                >
                  <span
                    v-for="(_url, idx) in regForm.giay_phep_urls"
                    :key="idx"
                    style="display: block; font-size: 0.9em; color: #1a7a4a"
                  >
            ...
            <div class="nd-field">
              ...
              <div class="nd-input-wrap">
                <textarea
                  v-model="regForm.thong_tin_xuat_hoa_don"
            ...
            <div class="nd-field">
              <label class="nd-label"
                >Mật khẩu <span class="nd-required">*</span></label
              >
              <div class="nd-input-wrap">
                <span class="nd-input-icon"
                  ><span class="material-symbols-outlined text-current"
            ...
            <div class="nd-field">
              <label class="nd-label"
                >Xác nhận mật khẩu <span class="nd-required">*</span></label
              >
              <div class="nd-input-wrap">
                <span class="nd-input-icon"
                  ><span class="material-symbols-outlined text-current"
            ...
            <div class="nd-info-box">
              ...
              >
              <span
                >Sau khi đăng ký, tài khoản sẽ được Admin xét duyệt trước khi
                hoạt động.</span
              >
            </div>

            <button type="submit" class="nd-btn-primary">Tiếp tục →</button>
          </form>

          <p class="nd-switch-text">
            Đã có tài khoản?
            <a href="#" @click.prevent="switchToLogin" class="nd-link"
              >Đăng nhập</a
            >
          </p>
        </template>
        ...
        <template v-if="mode === 'register' && step === 2">
          ...
          <div class="nd-confirm-card">
            ...
            <div class="nd-confirm-row">
              ...
              <span class="nd-confirm-value nd-role-farmer-tag"
                >🌾 Nông Dân</span
              >
            </div>
          </div>
          ...
          <div class="nd-btn-group">
            <button @click="step = 1" class="nd-btn-secondary">
              ← Quay lại
            </button>
            <button
              @click="handleRegister"
              :disabled="loading"
              class="nd-btn-primary nd-btn-flex"
            >
              <span v-if="loading" class="nd-spinner"></span>
