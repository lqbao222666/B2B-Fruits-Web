...
const confirmPayRemaining = async () => {
  ...
  try {
    ...
    notify.success(
      "Xác nhận đã nhận hàng & Thanh toán 85% còn lại thành công!",
    );
...
const isGroupSelected = (group: any) => {
  return (
    ...
    group.items.every((i: any) => selectedItemIds.value.includes(i.id))
  );
};
...
const totalCheckoutItemsCount = computed(() => {
  ...
  const itemsToCount = checkoutGroup.value
    ? checkoutGroup.value.items.filter((i: any) =>
...
const totalCheckoutShipping = computed(() => {
  ...
  const groupsToCalculate = checkoutGroup.value
    ? [checkoutGroup.value]
    : groupedCart.value.filter((g) =>
        g.items.some((i: any) => selectedItemIds.value.includes(i.id)),
      );

  let totalShipping = 0;
  groupsToCalculate.forEach((g) => {
    const selectedInGroup = g.items.filter((i: any) =>
      selectedItemIds.value.includes(i.id),
    );
    if (selectedInGroup.length > 0) {
      ...
      const farmerLat = firstItem.baiDang.latitude
        ? Number(firstItem.baiDang.latitude)
        : 10.762622;
      const farmerLng = firstItem.baiDang.longitude
        ? Number(firstItem.baiDang.longitude)
        : 106.660172;
...
const updateCheckoutTotals = () => {
  if (deliveryLocation.value) {
    const firstItem = checkoutGroup.value
      ? checkoutGroup.value.items[0]
      : cartItems.value[0];
...
const openCheckout = (group?: any) => {
  if (group) {
    const selectedInGroup = group.items.filter((i: any) =>
      selectedItemIds.value.includes(i.id),
    );
    if (selectedInGroup.length === 0) {
...
const initMap = () => {
  ...
  L.marker([farmerLat, farmerLng])
    .addTo(map)
    .bindPopup("Vị trí lấy hàng (Nông dân)")
    .openPopup();
...
const executeCheckout = async () => {
  ...
  try {
    ...
    for (const group of groupsToProcess) {
      ...
      const groupDist = haversineDistance(
        farmerLat,
        farmerLng,
        deliveryLocation.value!.lat,
        deliveryLocation.value!.lng,
  ...
  <main class="mx-auto max-w-[1000px] w-full px-4 py-8 font-sans">
    ...
    >
      <h1 class="text-3xl font-black text-slate-900 flex items-center gap-3">
        <span class="material-symbols-outlined text-4xl text-[#2E7D32]"
          >shopping_cart</span
        >
        Quản lý Giỏ hàng & Thanh toán lô hàng
      </h1>
      <button
        v-if="activeTab === 'cart' && cartItems.length > 0"
        @click="openCheckout()"
    ...
    <div v-if="activeTab === 'cart'">
      <div v-if="loading" class="flex justify-center py-20 text-slate-400">
        <span
          class="material-symbols-outlined animate-spin text-4xl text-[#2E7D32]"
          >sync</span
        >
      </div>

      <div
        v-else-if="groupedCart.length === 0"
        class="text-center py-32 bg-white rounded-3xl border border-slate-100 shadow-sm"
      >
        ...
        <button
          ...
          class="px-6 py-3 bg-[#2E7D32] hover:bg-[#1B5E20] text-white font-bold rounded-xl transition-colors"
        >
          Tiếp tục mua sắm
        </button>
      </div>
        ...
        >
          ...
          >
            ...
            <label class="flex items-center gap-2 cursor-pointer">
              ...
              <span class="text-sm font-bold text-slate-600"
                >Chọn tất cả loại trong lô</span
              >
            </label>
          </div>
          ...
          <div class="divide-y divide-slate-100">
            <div
              ...
              :key="item.id"
              class="p-6 flex flex-col md:flex-row gap-6 items-center hover:bg-slate-50/50 transition-colors"
            >
              <input
                type="checkbox"
              ...
              <div class="flex-1">
                ...
                <p class="text-sm font-bold text-[#d00000]">
                  {{ formatPrice(item.phanLoai.gia) }}
                  <span class="text-xs text-slate-500 font-normal"
                    >/ {{ item.baiDang.don_vi_tinh }}</span
                  >
                </p>
                <p class="text-xs text-slate-500 mt-1">
                  Còn lại: {{ item.phanLoai.so_luong_con_lai }}
                  {{ item.baiDang.don_vi_tinh }}
                </p>
              </div>
                ...
                <button
                  @click="
                    updateQuantity(
                      ...
                      -1,
                      Number(item.phanLoai.so_luong_con_lai),
                    )
                  "
                  class="w-8 h-8 rounded-full bg-slate-200 hover:bg-slate-300 flex items-center justify-center font-black transition-colors"
                >
                  -
                </button>
                <input
              ...
              <div class="w-32 text-right">
                <p class="font-black text-[#d00000]">
                  {{
                    formatPrice(
                      Number(item.so_luong) * Number(item.phanLoai.gia),
                    )
                  }}
                </p>
              </div>
              ...
              <button
                ...
                class="w-10 h-10 rounded-full bg-red-50 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors"
              >
          ...
          <div
            class="bg-slate-50 px-6 py-5 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4"
          >
            <div
              class="text-sm font-bold text-slate-600 flex flex-wrap items-center gap-4"
            >
              <span
                >Đã chọn:
                <strong class="text-[#2E7D32]">{{
                  group.items.filter((i: any) => isItemSelected(i.id)).length
                }}</strong>
                loại</span
              >
              <span>&bull;</span>
              <span
                >Tiền hàng lô:
                <strong class="text-[#d00000] text-base">{{
            ...
            <button
              ...
              class="px-8 py-3 bg-[#2E7D32] hover:bg-[#1B5E20] text-white font-bold rounded-xl transition-all shadow-md cursor-pointer flex items-center gap-2"
            >
    ...
    <div v-else-if="activeTab === 'orders'" class="space-y-6">
      <div
        v-if="loadingOrders"
        class="flex justify-center py-20 text-slate-400"
      >
        <span
          class="material-symbols-outlined animate-spin text-4xl text-[#2E7D32]"
          >sync</span
      ...
      <div
        ...
        class="text-center py-24 bg-white rounded-3xl border border-slate-100 shadow-sm"
      >
        <span class="material-symbols-outlined text-6xl text-slate-300 mb-4"
          >inventory_2</span
        >
      ...
      <div v-else class="space-y-4">
        <div
          ...
          class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between gap-6 items-start md:items-center"
        >
          <div class="space-y-2 flex-1">
            <div class="flex items-center gap-3">
              <span class="font-black text-slate-900 text-base"
                >Mã đơn: #{{ order.ma_don_hang || order.donhang_id }}</span
              >
              <span
            ...
            <p class="text-base font-bold text-slate-800">
              {{ order.baiDang?.tieu_de || order.baiDang?.ten_nong_san }}
            </p>
            <p class="text-xs text-slate-500">
              Giao tới: {{ order.dia_chi_giao }}
            </p>
            <div
              class="flex flex-wrap gap-4 text-xs font-semibold text-slate-600 pt-2 border-t border-slate-100"
            >
              <span
                >Tổng giá trị đơn:
              ...
              <span
                ...
                <strong class="text-[#2E7D32]">{{
                  formatPrice(
                    Number(order.tien_coc || Number(order.tong_tien) * 0.15),
                  )
                }}</strong></span
              >
              <span
                >Còn lại (85%):
                <strong class="text-[#d00000]">{{
                  formatPrice(
                    Number(order.tong_tien) -
                      Number(order.tien_coc || Number(order.tong_tien) * 0.15),
                  )
                }}</strong></span
              >
          ...
          <div class="flex flex-col gap-2 w-full md:w-auto">
            ...
            <div
              v-else
              class="text-xs font-bold text-green-700 bg-green-50 px-4 py-2 rounded-xl text-center border border-green-200 flex items-center gap-1.5 justify-center"
            >
              <span class="material-symbols-outlined text-sm"
                >check_circle</span
              >
    ...
    >
      ...
      >
        ...
        <div class="p-6 md:p-8">
          <div class="mb-6">
            <h2 class="text-2xl font-black text-slate-900 uppercase italic">
              Xác Nhận Đặt Cọc 15% & Tạo Đơn Hàng
            </h2>
            <p
              class="text-sm font-bold text-[#2E7D32] mt-1 flex items-center gap-1"
            >
              ...
              {{
                checkoutGroup
                  ? "Bài đăng lô hàng: " + checkoutGroup.tieu_de
                  : "Thanh toán gộp cho tất cả các lô hàng đã chọn trong giỏ"
              }}
            </p>
          </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-4">
              ...
              <div class="flex gap-2">
                <input
                  ...
                  type="text"
                  placeholder="Nhập địa chỉ (VD: Đồng Tháp) hoặc Tọa độ (lat, lng)..."
              ...
              <div
                id="checkout-map"
                class="w-full h-[320px] bg-slate-100 rounded-2xl border-2 border-slate-200 overflow-hidden relative z-0"
              ></div>

              <div
                v-if="deliveryLocation && !selectedSavedLocation"
              ...
              >
                <label class="flex items-center gap-2 cursor-pointer mb-2">
                  ...
                  <span class="text-sm font-bold text-slate-700"
                    >Lưu tọa độ kho này cho lần sau</span
                  >
                </label>
                <div v-if="isSavingLocation" class="flex gap-2">
                  <input
                    ...
                    type="text"
                    placeholder="Nhập tên gợi nhớ (VD: Kho hàng 1)..."
              ...
              <div
                v-if="distanceKm > 0"
                class="bg-[#E8F5E9] p-3.5 rounded-xl border border-[#2E7D32]/20 flex justify-between items-center"
              >
                <span class="font-bold text-[#2E7D32] text-sm"
                  >Khoảng cách tới vườn:</span
                >
                <span class="font-black text-xl text-[#2E7D32]"
            ...
            <div class="space-y-6 flex flex-col">
              <div>
                <p class="font-bold text-slate-700 text-sm mb-3">
                ...
                <div class="space-y-3">
                  <input
                    ...
                    class="w-full p-3 rounded-xl border border-slate-200 focus:border-[#2E7D32] outline-none text-sm font-medium"
                  />
                  <input
                  ...
                  <textarea
                    ...
                    placeholder="Địa chỉ chi tiết (để tài xế dễ tìm)"
                    class="w-full p-3 rounded-xl border border-slate-200 focus:border-[#2E7D32] outline-none text-sm font-medium h-20 resize-none"
                  ></textarea>
                </div>
              </div>
              ...
              <div
                class="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-3"
              >
                <h3
                  class="text-xs font-black text-slate-700 uppercase tracking-wider flex items-center gap-1"
                >
                  <span
                    class="material-symbols-outlined text-base text-[#2E7D32]"
                    >receipt_long</span
                  >
                ...
                >
                  ...
                  <span class="text-slate-900">{{
                    formatPrice(totalCheckoutGoods)
                  }}</span>
                </div>
                ...
                >
                  ...
                  <span
                    ...
                    >Vui lòng chọn vị trí kho</span
                  >
                  <span v-else class="text-slate-900">{{
                    formatPrice(totalCheckoutShipping)
                  }}</span>
                </div>

                <div
                  class="flex justify-between items-center pt-3 border-t border-slate-200 text-base font-black text-slate-900"
                >
                  <span>TỔNG GIÁ TRỊ ĐƠN (HÀNG + XE):</span>
                  <span class="text-[#d00000] text-xl">{{
                    formatPrice(totalCheckoutGoods + totalCheckoutShipping)
                  }}</span>
                </div>

                <div
                  class="bg-green-50 p-4 rounded-xl border border-green-200 space-y-1"
                >
                  <div class="flex justify-between items-center">
                    <span class="font-black text-[#2E7D32] text-sm uppercase"
                      >SỐ TIỀN CẦN ĐẶT CỌC (15% TỔNG HÀNG + XE):</span
                    >
                    <span class="font-black text-2xl text-[#2E7D32]">{{
                      formatPrice(
                        (totalCheckoutGoods + totalCheckoutShipping) * 0.15,
                      )
                    }}</span>
                  </div>
                  <p class="text-[11px] text-[#2E7D32]/80 italic">
                    * Đặt cọc 15% bao gồm cả tiền hàng và cước xe vận chuyển
                  </p>
                </div>

                <div
                  class="flex justify-between items-center text-xs font-bold text-slate-500 pt-1"
                >
                  <span
                    >Số tiền 85% còn lại (Thanh toán khi nhận & kiểm tra
                    hàng):</span
                  >
                  <span class="text-slate-800 font-black">{{
                    formatPrice(
                      (totalCheckoutGoods + totalCheckoutShipping) * 0.85,
                    )
                  }}</span>
                </div>
              </div>
              ...
              <button
                @click="startMockPayment"
                :disabled="isProcessing || !deliveryLocation"
                class="w-full mt-auto bg-[#2E7D32] hover:bg-[#1B5E20] disabled:bg-slate-300 text-white p-4 rounded-xl font-black uppercase tracking-wider transition-all shadow-md active:scale-95 cursor-pointer flex items-center justify-center gap-2"
              >
    ...
    >
      ...
      >
        <button
          ...
          class="absolute top-4 right-4 bg-slate-100 hover:bg-slate-200 p-2 rounded-full transition-colors"
        >
          <span class="material-symbols-outlined text-slate-600">close</span>
        </button>
        ...
        <div>
          <h2 class="text-xl font-black text-slate-900 mb-1">
            Xác Nhận Nhận Hàng & Thanh Toán 85%
          </h2>
          <p class="text-xs text-slate-500">
            Mã đơn hàng: #{{
              payingOrder.ma_don_hang || payingOrder.donhang_id
            }}
          </p>
        </div>

        <div
          class="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2 text-sm"
        >
          <div class="flex justify-between text-slate-600">
            <span>Tổng giá trị đơn hàng:</span>
            <span class="font-bold text-slate-800">{{
              formatPrice(Number(payingOrder.tong_tien))
            }}</span>
          </div>
          <div class="flex justify-between text-slate-600">
            ...
            <span class="font-bold text-[#2E7D32]"
              ...
              {{
                formatPrice(
                  Number(
                    payingOrder.tien_coc ||
                      Number(payingOrder.tong_tien) * 0.15,
                  ),
                )
              }}</span
            >
          ...
          <div
            class="pt-2 border-t border-slate-200 flex justify-between text-base"
          >
            <span class="font-black text-slate-900"
              >Số tiền 85% cần thanh toán:</span
            >
            <span class="font-black text-xl text-[#d00000]">{{
              formatPrice(
                Number(payingOrder.tong_tien) -
                  Number(
                    payingOrder.tien_coc ||
                      Number(payingOrder.tong_tien) * 0.15,
                  ),
        ...
        <div class="space-y-2">
          <label
            ...
            >Phương thức thanh toán 85% còn lại</label
          >
          <div class="grid grid-cols-2 gap-3 text-xs font-bold">
            ...
            <button
              ...
              "
            >
              <span class="material-symbols-outlined text-sm">wallet</span> Ví
              MoMo / App
            </button>
          </div>
        </div>
        ...
        <button
          @click="confirmPayRemaining"
          :disabled="isPayingRemaining"
          class="w-full bg-[#2E7D32] hover:bg-[#1B5E20] text-white p-4 rounded-xl font-bold uppercase tracking-wider transition-all shadow-md active:scale-95 disabled:opacity-50 cursor-pointer"
        >
          {{
            isPayingRemaining
              ? "Đang xử lý..."
              : "Xác Nhận Đã Kiểm Tra Hàng & Thanh Toán 85%"
          }}
        </button>
      </div>
    </div>
