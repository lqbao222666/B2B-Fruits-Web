  ...
  if (Array.isArray(images) && images.length > 0) {
    ...
    const imgUrl =
      typeof firstImage === "string"
        ? firstImage
        : firstImage.url || firstImage.image_url || firstImage.hinh_anh_url;
    if (imgUrl) {
      return imgUrl.startsWith("http")
        ? imgUrl
        : `${import.meta.env.VITE_API_URL || "http://localhost:3000"}${imgUrl}`;
    }
  }
...
const tabs = [
  ...
  { id: "da_giao_hang", label: "Đã giao tới nơi", icon: "where_to_vote" },
...
const getStatusInfo = (status: string) => {
  const map: Record<string, any> = {
    cho_xac_nhan: {
      label: "Chờ cọc & phí ship",
      color: "text-amber-600 bg-amber-50 border-amber-200",
      icon: "hourglass_empty",
      step: 0,
    },
    da_xac_nhan: {
      label: "Đã cọc & phí ship",
      color: "text-blue-600 bg-blue-50 border-blue-200",
      icon: "verified",
      step: 1,
    },
    dang_giao: {
      label: "Xe B2B đang giao",
      color: "text-indigo-600 bg-indigo-50 border-indigo-200",
      icon: "local_shipping",
      step: 2,
    },
    da_giao_hang: {
      label: "Đã giao tới nơi (Chờ trả 85%)",
...
<template>
  <div class="min-h-screen bg-slate-50 py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
      ...
      <div class="flex items-center justify-between mb-8">
        <div>
          ...
          <p class="text-slate-500 mt-1">
            Quản lý giao dịch và theo dõi tiến trình
          </p>
        </div>
        <button
          @click="fetchOrders"
          class="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-[#2E7D32] hover:border-[#2E7D32] transition-colors shadow-sm"
          title="Làm mới"
        >
          <span
      ...
      <!-- TABS -->
      <div
        class="bg-white rounded-2xl p-2 mb-6 border border-slate-200 shadow-sm overflow-x-auto hide-scrollbar"
      >
      ...
      >
        <div
          class="w-12 h-12 border-4 border-slate-200 border-t-[#2E7D32] rounded-full animate-spin"
        ></div>
      ...
      <div
        v-else-if="displayedOrders.length === 0"
        class="bg-white rounded-3xl border border-slate-200 p-16 flex flex-col items-center justify-center text-center shadow-sm"
      >
        <div
          class="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mb-6"
        >
          <span class="material-symbols-outlined text-[48px] text-slate-300"
            >inventory_2</span
          >
        </div>
        ...
        <p class="text-slate-500 max-w-sm mb-8">
          Bạn hiện tại chưa có giao dịch nào ở trạng thái "{{
            tabs.find((t) => t.id === activeTab)?.label
          }}".
        </p>
        <RouterLink
          to="/products"
          class="bg-[#2E7D32] text-white font-bold py-3 px-8 rounded-xl hover:bg-[#1B5E20] transition-colors shadow-lg shadow-green-900/20"
        >
      ...
      <div v-else class="grid gap-6">
        ...
        >
          ...
          <div
            v-if="order.trang_thai_don === 'da_xac_nhan'"
            class="absolute top-0 right-0 bg-blue-500 text-white text-[10px] font-black uppercase tracking-widest px-4 py-1 rounded-bl-xl z-10 shadow-sm flex items-center gap-1"
          >
          ...
          <div
            v-if="order.trang_thai_don === 'dang_giao'"
            class="absolute top-0 right-0 bg-indigo-500 text-white text-[10px] font-black uppercase tracking-widest px-4 py-1 rounded-bl-xl z-10 shadow-sm flex items-center gap-1"
          >
          ...
          <div class="flex justify-between items-start mb-5">
            <div class="flex items-center gap-3">
              <div
                class="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-[#E8F5E9] group-hover:text-[#2E7D32] group-hover:border-[#2E7D32]/20 transition-colors"
              >
              ...
              <div>
                <p class="text-sm text-slate-500 font-medium mb-0.5">
                  Mã đơn hàng
                </p>
                <p class="font-black text-slate-800 text-lg uppercase">
                  #{{ order.ma_don_hang || order.donhang_id }}
                </p>
              </div>
            </div>
            ...
            <div class="text-right">
              ...
              <div
                class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold"
                :class="getStatusInfo(order.trang_thai_don).color"
              >
                <span class="material-symbols-outlined text-[16px]">{{
                  getStatusInfo(order.trang_thai_don).icon
                }}</span>
          ...
          <div
            class="bg-slate-50 rounded-2xl p-4 flex flex-col md:flex-row gap-5 items-center justify-between border border-slate-100"
          >
            <!-- Product -->
            <div class="flex items-center gap-4 w-full md:w-1/2">
              <img
                v-if="!order.ma_don_hang?.startsWith('B2B-NC-')"
              ...
              <div class="flex-1">
                <h4
                  class="font-bold text-slate-800 text-base mb-1 line-clamp-1"
                >
                  {{ order.baiDang?.ten_nong_san || "Sản phẩm nông sản" }}
                </h4>
                <div
                  class="flex items-center gap-2 text-sm text-slate-500 mb-2"
                >
                  <span
                    class="bg-white px-2 py-0.5 rounded-md border border-slate-200 font-medium"
                    >{{ getTotalQuantity(order) }}
                    {{ order.baiDang?.don_vi_tinh || "kg" }}</span
                  >
                  <span
                    v-if="order.chiTiets?.length > 1"
                    class="bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md text-[11px] font-bold"
                  >
                ...
                <!-- Partner Info -->
                <p
                  class="text-xs font-medium text-slate-600 flex items-center gap-1"
                >
                  <span class="material-symbols-outlined text-[14px]"
                    >person</span
                  >
                  <span v-if="order.nguoi_mua_id === userId"
                    >Cung cấp bởi:
                    <span class="font-bold text-slate-800">{{
                      order.nguoiBan?.user?.full_name || "Nông dân"
                    }}</span></span
                  >
                  <span v-else
                    >Khách mua:
                    <span class="font-bold text-slate-800">{{
                      order.nguoiMua?.user?.full_name || "Doanh nghiệp"
                    }}</span></span
                  >
                </p>
              </div>
            ...
            <!-- Price Summary -->
            <div
              class="w-full md:w-auto flex md:flex-col justify-between md:justify-center items-center md:items-end gap-1 md:gap-2 px-4 md:px-0 py-2 md:py-0 border-t md:border-t-0 border-slate-200 mt-2 md:mt-0 pt-4 md:pt-0"
            >
              <div class="text-left md:text-right">
                ...
                <p class="text-2xl font-black text-[#d00000]">
                  {{ formatCurrency(order.tong_tien) }}
                </p>
              </div>
              <div
                v-if="order.tien_coc > 0 && order.trang_thai_don !== 'da_huy'"
                class="bg-[#E8F5E9] border border-[#2E7D32]/20 px-3 py-1.5 rounded-lg flex flex-col items-end"
              >
                <span class="text-[10px] font-bold text-[#2E7D32] uppercase"
                  >Tiền cọc cần thanh toán</span
                >
          ...
          <div class="mt-5 flex justify-end gap-3 flex-wrap" @click.stop>
            ...
            <button
              v-if="
                order.trang_thai_don === 'cho_xac_nhan' &&
                order.nguoi_ban_id === userId
              "
            ...
            <button
              v-if="
                order.trang_thai_don === 'da_xac_nhan' &&
                order.nguoi_ban_id === userId
              "
            ...
            <button
              v-if="
                (order.trang_thai_don === 'dang_giao' ||
                  order.trang_thai_don === 'da_xac_nhan') &&
                order.nguoi_mua_id === userId
              "
            ...
            <button
              v-if="
                order.nguoi_ban_id === userId &&
                !['hoan_thanh', 'da_huy'].includes(order.trang_thai_don)
              "
