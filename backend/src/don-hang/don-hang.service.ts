...
export class DonHangService {
  ...
  async datHang(user_id: number, dto: DatHangDto) {
    ...
    if (dto.hinh_thuc_giao_hang !== HinhThucGiaoHang.tu_den_lay) {
      const dist = (dto.khoang_cach !== undefined && dto.khoang_cach !== null) ? Number(dto.khoang_cach) : 0;
      const totalSoLuong = (dto.items || []).reduce((acc, item) => acc + Number(item.so_luong), 0);
  ...
  async create(createDto: CreateDonHangDto) {
    try {
      ...
      createDto.ma_xac_nhan = Math.floor(100000 + Math.random() * 900000).toString();
      ...
      if (createDto.hinh_thuc_giao_hang === HinhThucGiaoHang.tu_den_lay) {
        ...
        if (!createDto.khoang_cach) {
           throw new BadRequestException('Vui lòng cung cấp khoảng cách giao hàng');
        }
        createDto.phi_van_chuyen = this.calculateShippingFee(createDto.so_luong, createDto.khoang_cach);
      ...
      createDto.trang_thai_don = 'da_xac_nhan' as any;
      createDto.trang_thai_tt = 'da_thanh_toan' as any;

      return await this.repository.create(createDto);
    } catch (e) {
      require('fs').appendFileSync('error.log', new Date().toISOString() + ': ' + e.message + '\n' + e.stack + '\n');
      throw e;
    }
  }
