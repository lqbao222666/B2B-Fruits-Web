...
export class DanhGiaRepository {
  ...
  async create(nguoi_danhgia_id: number, data: CreateDanhGiaDto) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (isNaN(numericNguoiDanhGiaId) || numericNguoiDanhGiaId <= 0) {
        throw new BadRequestException('Xác thực tài khoản người đánh giá thất bại');
      }
      ...
      if (donHang.nguoi_mua_id !== numericNguoiDanhGiaId) {
        throw new BadRequestException('Bạn chỉ có quyền đánh giá đơn hàng do mình mua');
      }
      ...
      const danhGia = await tx.danhGia.create({
        data: {
          ...
          diem_chat_luong: data.diem_chat_luong ? Number(data.diem_chat_luong) : null,
      ...
      if (baidangAgg._avg.diem_tong !== null && baidangAgg._avg.diem_tong !== undefined) {
      ...
      if (nongDanAgg._avg.diem_tong !== null && nongDanAgg._avg.diem_tong !== undefined) {
        const ndExists = await tx.nongDan.findUnique({ where: { user_id: nguoi_duoc_dg_id } });
        if (ndExists) {
  ...
  async reply(danhgia_id: number, nguoi_tra_loi_id: number, data: ReplyDanhGiaDto) {
    ...
    const existing = await this.prisma.danhGia.findUnique({ where: { danhgia_id: id } });
