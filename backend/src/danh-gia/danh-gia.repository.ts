...
export class DanhGiaRepository {
  ...
  async create(nguoi_danhgia_id: number, data: CreateDanhGiaDto) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const donHang = await tx.donHang.findUnique({ where: { donhang_id: data.donhang_id } });
      ...
      const existing = await tx.danhGia.findUnique({ where: { donhang_id: data.donhang_id } });
      if (existing) {
      ...
      const danhGia = await tx.danhGia.create({
        data: {
          ...
          baidang_id: data.baidang_id,
          nguoi_danhgia_id,
          nguoi_duoc_dg_id: data.nguoi_duoc_dg_id,
          diem_tong: data.diem_tong,
          diem_chat_luong: data.diem_chat_luong,
          diem_dung_hen: data.diem_dung_hen,
          diem_thai_do: data.diem_thai_do,
      ...
      const nongDanAgg = await tx.danhGia.aggregate({
        where: { nguoi_duoc_dg_id: data.nguoi_duoc_dg_id },
        _avg: { diem_tong: true }
      });
      if (nongDanAgg._avg.diem_tong) {
        await tx.nongDan.update({
          where: { user_id: data.nguoi_duoc_dg_id },
  ...
  async reply(danhgia_id: number, nguoi_tra_loi_id: number, data: ReplyDanhGiaDto) {
    const existing = await this.prisma.danhGia.findUnique({ where: { danhgia_id } });
    if (!existing) throw new NotFoundException('Đánh giá không tồn tại');
    
    if (existing.nguoi_duoc_dg_id !== nguoi_tra_loi_id) {
  ...
  async findByNongDan(nguoi_duoc_dg_id: number) {
    return this.prisma.danhGia.findMany({
      ...
      include: {
        nguoiDanhGia: { select: { full_name: true, avatar_url: true } },
        baiDang: { select: { tieu_de: true, ten_nong_san: true, images: true } }
      },
      orderBy: { created_at: 'desc' }
    });
  }
